...
```
        ...
        LangChain Text Splitter
                     │
                     ▼
           data/chunks/xxx.json
                     │
                     ▼
            app/rag/embedder.py
                     │
     bge-small / DashScope Embedding
                     │
                     ▼
          data/vectors/xxx.json
                     │
                     ▼
    { source, total_vectors, vectors[] }
```
...
| 模块 | 文件 | 职责 |
|------|------|------|
| 配置层 | `app/core/config.py` | API、模型、上传/解析/切分/向量目录 |
| 契约层 | `app/models/schemas.py` | Pydantic 请求/响应模型 |
| 推理层 | `app/core/llm.py` | OpenAI 客户端 + `chat()` |
| 接口层 | `app/api/*.py` | REST：`/chat`、`/upload`、`/health` |
...
Phase 2（v0.2）   企业知识库 RAG
      Day08✅ 上传 → Day09✅ 解析 → Day10✅ 切分 → Day11✅ 向量 → Day12~13 入库/检索
      ↓
Phase 3           Agent + MCP
```
...
- [Day01.md](Day01.md) ~ [Day11.md](Day11.md) — 每日工作日志

---

*文档版本：v0.2.0-beta2 | 最后更新：2026-07*
